-- ============================================================
-- ProspectMoto — Novas funcionalidades
-- placa/modelo da moto, motivo "já fez o serviço" + local,
-- RPCs atualizadas e RPC de métricas do dashboard
-- ============================================================

-- ── Novas colunas ───────────────────────────────────────────
alter table public.service_records add column if not exists motorcycle_plate text not null default '';
alter table public.service_records add column if not exists motorcycle_model text not null default '';
alter table public.contact_attempts add column if not exists service_done_location text;
alter table public.abandonment_reasons add column if not exists is_service_done boolean not null default false;

-- Motivo especial "Já fez o serviço" (abre as 3 sub-opções no app)
insert into public.abandonment_reasons (label, is_other, is_service_done, sort_order)
select 'Já fez o serviço', false, true, 7
where not exists (select 1 from public.abandonment_reasons where is_service_done = true);

-- ── RPC: criar atendimento (agora com placa e modelo) ───────
drop function if exists public.create_service_record(text, text, bigint, text, date, int, int, int);
...
-- ── RPC: registrar resultado (agora com local do "já fez serviço") ──
drop function if exists public.register_contact_attempt(bigint, text, date, date, int, bigint, text);
...
create or replace function public.get_dashboard_metrics(p_from date, p_to date)
returns json language sql security definer set search_path = public as $$
  select json_build_object(
    'prospeccoes_registradas',
      (select count(*) from service_records where created_at::date between p_from and p_to),
    'agendamentos_realizados',
      (select count(*) from contact_attempts where outcome = 'scheduled' and created_at::date between p_from and p_to),
    'prospeccoes_sem_sucesso',
      (select count(*) from contact_attempts where outcome = 'abandoned' and created_at::date between p_from and p_to)
  );
$$;
